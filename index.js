...
*/
/*
cityData.reduce(function(currentValue))
*/
function processSum(numberList,callback) {
  /* CODE HERE */
  return callback( numberList.reduce( (accumulator, num) =>  accumulator + num, 0 ))}  // 0 is what index point to start
...
function firstNamesAllCaps(runners) {
  ...
  let firstName = []
   runners.forEach(runner => {
     const {first_name} = runner
     firstName.push(first_name.toUpperCase())
   })
  return firstName;
}
...
function counterMaker() {
  // BROKEN CODE STARTS
  let count = -1; // arrays start at zero, begin at less than one to start at zero
